...
async function downloadPDF() {
  ...
  // 🔥 Fix layout timing (important)
  await new Promise(resolve => setTimeout(resolve, 300));

  // 🔥 FIX: Prevent note section cutting
  const noteBox = document.getElementById("noteBox");
  if (noteBox) {
    const rect = noteBox.getBoundingClientRect();
    const pageHeightPx = 1122; // A4 height in px approx

    if (rect.top % pageHeightPx > pageHeightPx - 220) {
      noteBox.style.marginTop = "220px";
    }
  }
  ...
  if(finalSection) {
    ...
    const positionInPage = rect.top % pageHeightPx;

    if(positionInPage > pageHeightPx - 300) {
      finalSection.style.marginTop = "300px";
    }
  }
  ...
  // Additional pages (fixed slicing)
  while (heightLeft > 0) {
